...
typedef struct {
  ...
  unsigned int n_temps;         // number of tempered distributions to use
...
typedef struct {
  ...
  int (*evaluate)(const mcmc_model *, markov_chain *);
  int (*proposal)(const mcmc_model *, markov_chain *, double *, double *);
