    ...
    while ~Success
        ...
        if isempty(Model.Paras)
            ...
            for n = 1:Model.NumOfParas
                Chain{ChainNum}.Paras(n) = SamplePrior(Chain{ChainNum}.Priors{n});
            end
        else
            ...
            if Model.InferICs
                ...
                if isempty(Model.ICs)
                    ...
                    for n = 1:Model.NumOfSpecies
                        Chain{ChainNum}.ICs(n) = SamplePrior(Chain{ChainNum}.Priors_ICs{n});
                    end
                else
...
function [Chain, Success] = ODE_Calculate_Geometry_ICs(Model, Chain)

% Inputs: Model object, Chain object
% Output: Chain object, Success flag

Success = 1; % initialise as successful and change later if otherwise
...
% Calculate the log prior for each IC
for SpeciesNum = 1:NumOfSpecies
    Prior(SpeciesNum) = GetPrior(Model.PriorInfo_ICs, SpeciesNum, ICs(SpeciesNum));
end

if min(Prior) > 0
    Chain.LogPrior_ICs = log(Prior);
else
    % Proposed parameter values lie outside prior, so fail
    Success = 0;
    return
end
...
if strcmp(Model.Sampler_ICs, 'MH')
    %%% Only need to solve standard ODE %%%
    
    try
        % Solve the 1st order equations
        simdata = feval(Model.SBModelName, Model.TimePoints, ICs, Paras, Model.Options);
    catch
        Success = 0;
        return
    end
    ...
    for n = Model.ObservedSpecies
        Chain.LL(n) = LogMultNorm(StateEstimates(n,:)', Model.Data(n,:)', Model.NoiseCov{n});
    end
    
else
    %%% Solve augmented ODE to get 1st order sensitivities %%%
    
    % Augment the initial conditions - the IC wrt itself is 1...
    Temp = eye(NumOfSpecies);
    ICs = [ICs Temp(:)'];
    
    try
        % Solve the 1st order equations
        simdata = feval(Model.SBModelName_1st_ICs, Model.TimePoints, ICs, Paras, Model.Options);
    catch
        Success = 0;
        return
    end
    ...
    Chain.GradLL_ICs       = zeros(1, NumOfSpecies);
    Chain.GradLogPrior_ICs = zeros(1, NumOfSpecies);
    
    for SpeciesNum = 1:NumOfSpecies
        for i = Model.ObservedSpecies
            Chain.GradLL_ICs(SpeciesNum) = Chain.GradLL_ICs(SpeciesNum) - ((StateEstimates(i,:)-Model.Data(i,:))/Model.NoiseCov{i})*Sensitivities{SpeciesNum}(:,i);
        end
        
        % Calculate the gradient of the log prior
        Chain.GradLogPrior_ICs(SpeciesNum) = GetPriorLogDeriv(Model.PriorInfo_ICs, SpeciesNum, ICs(SpeciesNum));
    end
    
    
    % Now calculate metric tensor if necessary for sampler
    if strcmp(Model.Sampler_ICs, 'mMALA') || strcmp(Model.Sampler_ICs, 'Simp_mMALA')
        
        Chain.FI_ICs = zeros(NumOfSpecies);
        
        for i = 1:NumOfSpecies
            for j = i:NumOfSpecies
                for SpeciesNum = Model.ObservedSpecies %1:N
                    Chain.FI_ICs(i,j) = Chain.FI_ICs(i,j) + ((Sensitivities{i}(:,SpeciesNum)'/Model.NoiseCov{SpeciesNum})*Sensitivities{j}(:,SpeciesNum));
                end
                
                Chain.FI_ICs(j,i) = Chain.FI_ICs(i,j);
            end
            
        end
        
        % Calculate the Hessian of the log prior
        Chain.HessianLogPrior_ICs = GetPriorLog2ndDeriv(Model.PriorInfo_ICs, Chain.ICs);
