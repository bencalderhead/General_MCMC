...
int main() {
  ...
  CU_pSuite suite;
  if ((suite = CU_add_suite("normal", test_init, test_cleanup)) == NULL) {
...
static void test_params() {
  ...
  gmcmc_prior * normal;
  CU_ASSERT(gmcmc_prior_create(&normal, gmcmc_prior_normal, 0.0,  0.0) == GMCMC_ERROR_INVALID_ARGUMENT);
  CU_ASSERT(gmcmc_prior_create(&normal, gmcmc_prior_normal, 0.0, -1.0) == GMCMC_ERROR_INVALID_ARGUMENT);
...
static void test_evaluation() {
  ...
  gmcmc_prior * normal;
  CU_ASSERT(gmcmc_prior_create(&normal, gmcmc_prior_normal, 0.0, 1.0) == GMCMC_SUCCESS);

  CU_ASSERT_DOUBLE_EQUAL(gmcmc_prior_evaluate(normal, 0.0), 0.39894229, 1.e-8);
  CU_ASSERT_DOUBLE_EQUAL(gmcmc_prior_evaluate_1st_order(normal, 0.0), 0.0, 0.0);
  CU_ASSERT_DOUBLE_EQUAL(gmcmc_prior_evaluate_2nd_order(normal, 0.0), -1.0, 0.0);

  gmcmc_prior_destroy(normal);
}
...
static void test_statistics() {
  ...
  for (size_t i = 0; i < 100000; i++) {
    x[i] = gmcmc_prior_sample(normal, rng);
  ...
  CU_ASSERT_DOUBLE_EQUAL(mean, 0.0, 0.00159307);
  CU_ASSERT_DOUBLE_EQUAL(var, 1.0, 0.00194299);
